...
export interface CampaignCardProps {
  ...
  description: string;
  image: string;
  status: "vote" | "join" | "closed" | "comming";
}
const CampaignCard = ({
  title,
  description,
  ...
  const renderStatus = () => {
    if (status === "vote") {
      return (
        <div className="absolute right-2 -top-4 rounded-full h-10 w-10 bg-gradient-to-r from-pink to-light-pink to-50% flex items-center">
          <StarIcon className="h-6 w-6 text-white mx-auto" />
        </div>
      );
    }
    if (status === "join") {
